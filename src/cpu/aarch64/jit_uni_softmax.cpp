...
#define STR_PARAM_TMP(x, y) \
	...
	ldr(XReg(IDX(reg_spat_offt_count)), pre_ptr(X_DEFAULT_ADDR, PARAM_OFF(spat_offt_count)));
...
#undef PARAM_OFF
    ...
    void uni_fmax(const ZReg &dst, const ZReg &src, const ZReg &src2, const PReg &mask) {
	...
	printf("axis_loop\n");
        ...
        eor(reg_spat_offt, reg_spat_offt, reg_spat_offt); // spat_offt to get addr of src/dst
        L(main_loop);
        {
            if (n_loops_) {
                ...
                sub(reg_reverse_spat_offt, reg_reverse_spat_offt, unroll_regs_ * axis_stride_);
                add(reg_spat_offt, reg_spat_offt, unroll_regs_ * axis_stride_);
                b(main_loop);
            }
        }
    ...
    virtual void get_horizontal_op(const ZReg &v, const ZReg &vtmp, op_t op) = 0;
...
struct jit_softmax_t<sve_512> : public jit_softmax_base_t<sve_512> {
    ...
    void store(const XReg &addr, const ZReg &vmm, bool tail = false) {
        if (tail) st1w(ZRegS(IDX(vmm)), PReg(IDX(tail_opmask)) / T_m, ptr(XReg(IDX(addr)))); 
	else str(ZReg(IDX(vmm)), ptr(XReg(IDX(addr))));
    };

    void load(const ZReg &vmm, const XReg &addr, bool tail = false) {
        if (tail) ld1w(ZRegS(IDX(vmm)), PReg(IDX(tail_opmask)) / T_z, ptr(XReg(IDX(addr)))); 
	else ldr(ZReg(IDX(vmm)), ptr(XReg(IDX(addr))));
    };
	...
	printf("accumulate_vsum\n");
        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (tail)
		    fadd(ZRegS(IDX(vsum)), PReg(IDX(tail_opmask)) / T_m, ZRegS(IDX(vreg_tmp_src)));
                else
		    fadd(ZRegS(IDX(vsum)), ZRegS(IDX(vsum)), ZRegS(IDX(vreg_tmp_src)));
        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (is_softmax_) {
                    ...
                    fmul(ZRegS(IDX(vreg_tmp_src)), ZRegS(IDX(vreg_tmp_src)), ZRegS(IDX(vsum)));
                }
                if (is_logsoftmax_) {
                    load(vreg_tmp_src, dst_ptr(axis_stride_ * i), tail);
        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (is_softmax_) {
                    ...
                    fmul(ZRegS(IDX(vreg_tmp_diff_dst)), ZRegS(IDX(vreg_tmp_diff_dst)), ZRegS(IDX(vreg_tmp_dst)));
                }
		fadd(ZRegS(IDX(vsbr)), ZRegS(IDX(vsbr)), ZRegS(IDX(vreg_tmp_diff_dst)));
            }
        });
	...
	printf("compute_diff_src\n");
        axis_loop([&](int unroll, bool tail = false) {
		    ...
		    fmul(ZRegS(IDX(vreg_tmp_diff_dst)), ZRegS(IDX(vreg_tmp_dst)), ZRegS(IDX(vreg_tmp_diff_dst)));
                }
                if (is_logsoftmax_) {
                    exp_injector_->compute_vector(vreg_tmp_dst.getIdx());
		    fmls(ZRegS(IDX(vreg_tmp_diff_dst)), p_512 / T_m, ZRegS(IDX(vreg_tmp_dst)), ZRegS(IDX(vsbr)));
                }
...
status_t jit_uni_softmax_fwd_t<isa>::init(engine_t *engine) {
    CHECK(safe_ptr_assign(softmax_driver_, new softmax_impl::driver_t<isa>(pd())));
